...
/** agentId names the agent the step is about, when it is about one; the overview shows a step only for the agent on screen. */
export type NextStep = { key: "connect" | "check" | "progress" | "failed" | "first_test" | "new_version"; title: string; body: string; href: string; cta: string; agentId?: string };
...
export function nextStep(input: { agents: AgentRecord[]; versions: VersionRecord[]; jobs: Job[]; runs: LabRunSummary[] }): NextStep | null {
  ...
  if (active) return { key: "progress", agentId: active.request.agentId, title: `Test in progress: ${active.subject.name} ${active.subject.version}`, body: `${active.progress.completed} of ${active.progress.total} trials complete.`, href: `/labs/jobs/${active.id}`, cta: "View progress" };

  const latestJob = [...input.jobs].reverse().find((j) => j.request.agentId);
  if (latestJob && latestJob.state === "failed") return { key: "failed", agentId: latestJob.request.agentId, title: `The last test could not run: ${latestJob.subject.name} ${latestJob.subject.version}`, body: latestJob.error?.message ?? "The engine could not run the suite.", href: `/labs/jobs/${latestJob.id}`, cta: "Review run error" };
  ...
  for (const a of agents) {
    ...
    if (!hasRun) {
      ...
      return { key: "first_test", agentId: a.id, title: `Run the first test on ${a.name}`, body: "The starter suite runs every scenario in the open pool against your endpoint in a simulated world.", href: v ? `/labs/tests/new?agentId=${encodeURIComponent(a.id)}&versionId=${encodeURIComponent(v.id)}` : `/labs/agents/${a.id}`, cta: "Run first test" };
    }
  }
  ...
  for (const a of agents) {
    ...
    if (latest && !input.runs.some((r) => r.agent.registry?.versionId === latest.id)) {
      return { key: "new_version", agentId: a.id, title: `${a.name} ${latest.label} has not been tested`, body: "A new version has no evidence of its own; earlier results describe the earlier configuration.", href: `/labs/tests/new?agentId=${encodeURIComponent(a.id)}&versionId=${encodeURIComponent(latest.id)}`, cta: "Test new version" };
    }
  }
  return null;
}
