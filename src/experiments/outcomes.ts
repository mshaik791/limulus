...
/**
 * What varies between arms here is *guidance*: whether the agent is told how to
 * behave. That is a different axis from src/sandbox/controls.ts, where what
 * varies is *enforcement*: whether the rail will accept an order that skipped
 * the gate.
 *
 * The two are easy to confuse because both have three arms and both measure
 * something called "skipped the control". They are not the same metric:
 *
 *   here          every condition lets the agent skip, so a boolean is right
 *   controls.ts   the enforced arm makes skipping impossible, so it is null
 *                 there and must never render as 0
 *
 * The `limulus` condition below is an *advisory* gate — the rail accepts orders
 * that never reached it — which is why an agent could skip it in one trial of
 * twenty. See the 2026-09-21 entry in FINDINGS.md.
 */
